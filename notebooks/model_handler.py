...
from eval_utils import AverageMeter
from data_utils import sanitize_input, vectorize_input
from graph_utils import plot_metrics, plot_loss
...
class ModelHandler(object):
    ...
    def __init__(self, config):
        ...
        if train_set:
            self.train_loader = DataLoader(train_set, batch_size=config['batch_size'], shuffle=config['shuffle'], collate_fn=lambda x: x, pin_memory=True)
    ...
    def train(self):
        ...
        while self._stop_condition(self._epoch):
            ...
            # Add loss, F1, and em of train into plot data list
            self.plot_train_loss.append(self._train_loss.mean())
            self.plot_train_f1.append(self._train_f1.mean())
            self.plot_train_em.append(self._train_em.mean())
            ...
            # Add loss, F1, and em of dev into plot data list
            self.plot_dev_f1.append(self._dev_f1.mean())
            self.plot_dev_em.append(self._dev_em.mean())
            ...
            plot_metrics('F1', self.plot_train_f1, self.plot_dev_f1, self._epoch)
            plot_metrics('EM', self.plot_train_em, self.plot_dev_em, self._epoch)
            plot_loss(self.plot_train_loss, self._epoch)
